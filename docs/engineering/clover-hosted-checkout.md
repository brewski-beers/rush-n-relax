...
**Both full and partial refunds are supported via API.** Dashboard-only is _not_ a constraint.
...
> Status as of 2026-05-02. The webhook route at `apps/web/src/app/api/webhooks/clover/route.ts` is a **stub**, retained for the Path A future. **We are running Path B in production** — payment confirmation is performed by the customer-return path, not by webhooks.

### What runs today (Path B)

- **Outbound:** `apps/web/src/app/api/checkout/session/route.ts` (#277) creates a Clover Hosted Checkout session using `CLOVER_MERCHANT_ID` + `CLOVER_API_KEY` and redirects the customer to the Clover-hosted page.
- **Inbound:** the customer is redirected back to a return URL on the storefront. Today there is no code on that return path that reconciles `awaiting_payment → paid` — that work is **#279** (return-URL reconciliation route + 5-min recovery cron).
- **Refunds:** issued via Clover dashboard or, when scripted, against `POST /v1/refunds` on the Ecommerce API using the merchant's private token. The webhook stub does **not** observe these.

### The webhook stub (`/api/webhooks/clover/route.ts`)

The route exists and verifies HMAC against `CLOVER_WEBHOOK_SECRET` (when set), then maps Path-A-shaped events (`payment.succeeded` / `payment.failed` / `payment.refunded` / `payment.voided`) to order transitions via `transitionStatus`. It is **dormant in production** because:

1. We have no `CLOVER_WEBHOOK_SECRET` set on Path B (no Developer App, no app-level webhook).
2. Clover's actual webhook payload is CRUD-on-object (`P:CREATE` etc.) — see §1 above. The stub's `EVENT_TO_STATUS` map does not match what a real Path A integration would deliver. **Treat the stub as scaffolding, not a working handler.**

### Supersession plan

- **#279** introduces the return-URL reconciliation route (Path B). When merged, it becomes the source of truth for `awaiting_payment → paid | failed`. The stub remains untouched.
- **#302** moves us to Path A (TBB-built Developer App, OAuth install, app-level webhook secret). At that point the stub is rewritten to match Clover's real `(type, objectType)` shape — fan in on `/api/webhooks/clover`, branch on object type, GET the payment by id to read `result`. The current `EVENT_TO_STATUS` map is discarded.

### Runbook reference (Path B)

| Item              | Value                                                                                             |
| ----------------- | ------------------------------------------------------------------------------------------------- |
| Webhook URL       | `https://<host>/api/webhooks/clover` (stub — dormant on Path B)                                   |
| Signature header  | `x-clover-signature` (stub) → real Clover header is `X-Clover-Auth` and only fires on Path A      |
| Test mode         | Sandbox merchant `FFG76YKEA9QK1` + `CLOVER_BASE_URL=https://apisandbox.dev.clover.com`            |
| Env vars (live)   | `CLOVER_MERCHANT_ID`, `CLOVER_API_KEY`, `CLOVER_BASE_URL` — see env-vars table at top of this doc |
| Env vars (Path A) | `CLOVER_WEBHOOK_SECRET` will be required once the stub becomes a real handler (#302)              |
